...
# Force a fresh data_pipeline import bound to the config stub above. Without
# this, if another test module (e.g. test_resolution.py) already imported
# data_pipeline against a different fake config, the `sys.modules['config']`
# override two lines up would be too late — data_pipeline's top-level
# `from config import FIXED_VERSION_RULES` already ran against the OTHER
# module's stub, and setting sys.modules['config'] here doesn't retroactively
# change already-bound globals in an already-imported module.
