...
def load_previous_report(file_path):
    """
    Load the previous report for trend comparison.

    Accepts two formats:
      1. A dashboard .xlsx produced by a prior run — reads the 'Raw Data' or
         'All Detections' sheet as before.
      2. A *_RawData.csv sidecar produced by the CSV-write path introduced in
         v0.28 — read directly with pd.read_csv, skipping the ExcelFile step.

    Both formats return the same (df, resolved_pairs) tuple.  resolved_pairs
    is always empty for CSV sidecars (checkbox state is not stored in the CSV),
    which is correct: the CSV is raw scanner data, not a triage worksheet.
    """
    ...
    if _path.suffix.lower() == '.csv':
        try:
            ...
            raise ValueError(
                f"'{_path.name}' is currently open in another application.\n\n"
                "Please close the file and try again."
            )
        except FileNotFoundError:
            raise ValueError(
                f"'{_path.name}' could not be found.\n\nPlease check the file path and try again."
            )
        except Exception as e:
            raise ValueError(f"Could not open '{_path.name}'.\n\nDetails: {e}")

        missing = {'Name', 'Vulnerability Name'} - set(df.columns)
        if missing:
            raise ValueError(
                f"Previous report CSV '{_path.name}' is missing columns: "
                f"{', '.join(sorted(missing))}.\n"
                "Please supply a *_RawData.csv generated by this tool."
            )
        ...
        df['Vulnerability Score'] = pd.to_numeric(
            df.get('Vulnerability Score', 0), errors='coerce'
        ).fillna(0)

        # CSV sidecars carry no checkbox state — resolved_pairs is always empty.
        return df, set()

    # ── Excel (.xlsx) path — unchanged from previous implementation ────────────
    try:
        xl = pd.ExcelFile(file_path, engine=_XLSX_ENGINE)
    except PermissionError:
