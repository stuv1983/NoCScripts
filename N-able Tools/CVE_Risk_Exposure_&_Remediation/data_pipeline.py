"""
data_pipeline.py — data loading, merging, patch matching, and trend comparison.
...
# ==============================================================================
# PRE-COMPILED REGEX  (compiled once at import, reused for every row)
# ==============================================================================
...
def get_base_product(prod_name: str) -> str:
    """Strip architecture tags and trailing version numbers from a product name."""
...
def clean_sheet_name(name: str, used_names: Set[str]) -> str:
    """Return a valid, unique Excel sheet name (max 31 chars, no forbidden chars)."""
...
def determine_device_type(os_string: str) -> str:
    """Classify a device as 'Server', 'Workstation', or 'Unknown' from its OS string."""
...
# _norm_compact: remove all non-alphanumeric chars (for strict key matching).
# _norm_text:    replace non-alphanumeric runs with a space (for substring search).
...
def _get_arch(text: str) -> str:
    """Return 'x64', 'x86', or '' based on the architecture tag in text."""
...
def load_previous_report(file_path):
    ...
    if _is_dashboard:
        ...
        log.info(
            "Previous report loaded as dashboard output ('%s', sheet '%s'): "
            "%d rows, %d checkbox-resolved pairs",
            fname, target, len(df), len(resolved_pairs),
        )
    ...
    if missing:
        if _is_inventory:
            _hint = (
                f"'{fname}' looks like a Device Inventory / RMM report. "
                "For the Previous Report field, select either a dashboard produced "
                "by this tool or a raw CVE detections export from N-able."
            )
        else:
            _hint = (
                f"'{fname}' is missing required columns: {', '.join(sorted(missing))}.\n"
                "The Previous Report field accepts:\n"
                "  • A dashboard .xlsx produced by a prior run of this tool, OR\n"
    ...
    if 'Vulnerability Score' not in df.columns and 'Vulnerability Severity' in df.columns:
        df['Vulnerability Score'] = (
            ...
            .map(_SEVERITY_SCORE_MAP).fillna(0.0)
        )
    elif 'Vulnerability Score' not in df.columns:
        df['Vulnerability Score'] = 0.0

    df['Vulnerability Score'] = pd.to_numeric(df['Vulnerability Score'], errors='coerce').fillna(0)
    df['_Name_Key'] = _normalize_device_col(df['Name'])
    df['_CVE_Key']  = df['Vulnerability Name'].astype(str).apply(extract_cve_id)

    if 'Base Product' not in df.columns and 'Affected Products' in df.columns:
        df['Base Product'] = df['Affected Products'].apply(get_base_product)

    log.info(
        "Previous report loaded as raw CVE export ('%s', sheet '%s'): %d rows.  "
        "Rows absent from current report will be treated as resolved.",
        fname, target, len(df),
    )
...
def _active_trend_scope(df: pd.DataFrame, threshold: float,
                        ...
                        stale_devices=None,
                        is_raw_export: bool = False) -> pd.DataFrame:
    """
    ...
    Applies the full pipeline in one place so every caller uses identical logic:
      ...
      • UNRESOLVED-only (status column named 'Threat Status' or 'Status')
        Exception: when is_raw_export=True and no status column is present,
        all rows are treated as unresolved (raw exports are pre-filtered).
    ...
    if 'Base Product' not in out.columns:
        out['Base Product'] = (
            out['Affected Products'].apply(get_base_product)
            if 'Affected Products' in out.columns
            else ''
        )
...
def compute_trends(current_df, previous_df, threshold,
                   ...
                   prev_source_type: str = 'dashboard'):
    """
    ...
    prev_source_type: 'dashboard' | 'raw_export'
        When 'raw_export', the previous DataFrame may contain only unresolved rows
        (no Threat Status / Status column). _active_trend_scope is told not to
        apply an UNRESOLVED filter in that case — all rows are already unresolved.
    """
    ...
    prev_t = _active_trend_scope(previous_df, threshold, inventory_devices, stale_devices,
                                  is_raw_export=_prev_is_raw)

    if inventory_devices or stale_devices:
        dropped = len(_active_trend_scope(previous_df, threshold, is_raw_export=_prev_is_raw)) - len(prev_t)
        if dropped > 0:
